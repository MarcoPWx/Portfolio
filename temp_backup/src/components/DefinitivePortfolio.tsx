...
import { 
  ...
  ChevronDown, Info, Book, Workflow, Timer
} from 'lucide-react';
...
function CodeEditor({ code, language = 'javascript', title = 'example.js', showLineNumbers = true }) {
  ...
  const highlightCode = (code: string) => {
    return code
      ...
      .replace(/\b(const|let|var|function|return|if|else|for|while|import|export|default|from|async|await|class|extends|implements|interface|type)\b/g, '<span class="text-purple-400">$1</span>')
      .replace(/\b(true|false|null|undefined)\b/g, '<span class="text-orange-400">$1</span>')
      .replace(/\b(\d+)\b/g, '<span class="text-cyan-400">$1</span>')
      .replace(/(\/\/.*$|#.*$)/gm, '<span class="text-gray-500">$1</span>')
      .replace(/(@\w+)/g, '<span class="text-yellow-400">$1</span>')
      .replace(/\b(supabase|AuthProvider|useAuth|useState|useEffect|create|persist|HybridCache|VectorHybridStore|GamificationService|LegalComplianceEngine|PragmaticAIRouter|PBML)\b/g, '<span class="text-blue-400">$1</span>');
  };
...
export function DefinitivePortfolio() {
  ...
  const [activeProject, setActiveProject] = useState('quizmentor');
  ...
  const projectStatus = {
    quizmentor: {
      ...
      userJourney: {
        ...
        target: 'Notification → Open app → Daily challenge → Take quiz → Earn XP → Check leaderboard → Share achievement → Set goal → Return tomorrow',
        journeyCompletion: {
          'Onboarding': '0%',
          'Daily Quiz Flow': '40%', 
    ...
    harvest: {
      ...
      userJourney: {
        ...
        target: 'Landing → Sign up → Add API keys → Input URL → Select format → See cost estimate → Generate → Review → Export → Track usage',
        journeySteps: {
    ...
    auth: {
      ...
      userJourney: {
        ...
        target: 'Sign up → Verify email → Choose tier → Access products → Manage subscription → Use SSO across apps',
        features: {
          'Email/Password Auth': '✅',
          'Social Login': '✅',
    ...
    omni: {
      ...
      userJourney: {
        ...
        target: 'Install → Add API keys → Select provider → Start coding → Switch providers → Compare results',
        phases: {
  ...
  const projects = {
    quizmentor: {
      ...
      description: 'Revolutionary AI pair programming system with PBML (Pattern-Based Machine Learning) that actually learns from your codebase. Features hybrid Redis+Supabase architecture for sub-50ms responses.',
...
export class PBMLEngine {
  ...
  async suggestCode(context: CodeContext): Promise<Suggestion[]> {
    ...
    quizmentor: {
      ...
      description: 'Mobile learning platform with sophisticated psychological manipulation for maximum engagement. Currently 15% complete, needs 4-6 weeks with 3-4 developers.',
  ...
  async awardXP(baseXP: number): Promise<XPResult> {
    ...
    return { xp: totalXP, bonuses, levelUp: newLevel > this.currentLevel };
      ...
      architecture: {
        ...
        'What\'s Missing': ['Authentication', 'Database', 'Service connections', 'Any real functionality'],
    ...
    harvest: {
      ...
      description: 'Content intelligence platform that costs $50-100/month per user to run. Currently 20% complete, needs 16-20 weeks with 5 developers.',
...
class LegalComplianceEngine:
    ...
    auth: {
      ...
      description: 'Unified authentication system that\'s 70% complete and could actually ship. The most mature project in the ecosystem.',
...
export const useAuthStore = create<AuthState>()(
  persist(
    ...
    omni: {
      ...
      description: 'VS Code extension concept that exists only as documentation. Would unify AI providers if someone actually built it.',
...
class SmartRouter {
  ...
  const currentStatus = projectStatus[activeProject];

  // User Journey Modal Component
  const UserJourneyModal = ({ project, onClose }) => {
    const journey = projectStatus[project].userJourney;
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
          ...
          <div className="space-y-6">
            ...
            {journey.journeyCompletion && (
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(journey.journeyCompletion).map(([step, completion]) => (
                    <div key={step} className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-500 w-10 text-right">{completion}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {journey.journeySteps && (
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(journey.journeySteps).map(([step, data]) => (
            ...
            {journey.features && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(journey.features).map(([feature, status]) => (
                    <div key={feature} className="flex justify-between items-center bg-gray-800/50 rounded p-2">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            >
              ...
              <p className="text-xl text-gray-400 max-w-3xl mx-auto mb-8">
                A portfolio of ambitious AI projects in various stages of incompletion. 
                Currently serving 0 developers because nothing is deployed.
              </p>
              ...
              <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 mb-8 max-w-2xl mx-auto">
                ...
                <p className="text-sm text-gray-300">
                  Despite impressive documentation, most projects are 15-20% complete. 
                  QuizMentor has no auth, Harvest.ai would bankrupt users, and Omni.ai is just a README.
                </p>
              </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {impactMetrics.technical.map((item, idx) => (
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {impactMetrics.business.map((item, idx) => (
                  <div key={idx} className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
                    <div className="text-3xl font-bold text-white mb-1">{item.value}</div>
                    <div className="text-sm text-gray-400 mb-2">{item.metric}</div>
                    <div className="text-xs text-gray-500">{item.benchmark}</div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {impactMetrics.scale.map((item, idx) => (
                  <div key={idx} className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
                    <div className="text-3xl font-bold text-white mb-1">{item.value}</div>
                    <div className="text-sm text-gray-400 mb-2">{item.metric}</div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-8 mb-12">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <h4 className="text-lg font-semibold mb-4 text-yellow-400">What We Tell People</h4>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {Object.entries(realityCheck.breakdown).map(([project, data]) => (
                <div key={project} className="bg-gray-900/50 border border-gray-800 rounded-lg p-6">
                  <h4 className="text-lg font-bold mb-3 capitalize flex items-center gap-2">
                    {projects[project].icon && <projects[project].icon className="w-5 h-5" />}
                    {project}
                  </h4>
                  <div className="space-y-2">
                    <div>
            ...
            >
              ...
              <div className="text-center mb-8">
                <div className="flex justify-center items-center space-x-3 mb-4">
                  <div className={`w-12 h-12 bg-gradient-to-br ${currentProject.gradient} rounded-lg flex items-center justify-center`}>
              ...
              <div className="flex justify-center mb-8">
                <div className="inline-flex bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg p-1">
                  {['reality', 'psychology', 'code', 'journey'].map((tab) => (
                    ...
                    >
                      {tab === 'reality' ? 'Reality Check' : 
                       tab === 'psychology' ? 'Dark Patterns' :
              ...
              <div className="max-w-5xl mx-auto">
                {activeTab === 'reality' && (
                  <div className="space-y-6">
                    <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-6">
                      ...
                      <p className="text-gray-300 mb-2">Completion: {currentStatus.reality.completion}</p>
                      <p className="text-gray-300">Time to Production: {currentStatus.reality.timeToProd}</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <ul className="space-y-2">
                          {currentStatus.reality.actuallyWorks.map((item, idx) => (
                      ...
                      <div>
                        ...
                        <ul className="space-y-2">
                          {currentStatus.reality.doesNotWork.map((item, idx) => (
                            <li key={idx} className="text-sm text-gray-300">{item}</li>
                          ))}
                        </ul>
                      </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {currentStatus.reality.criticalBlockers.map((blocker, idx) => (
                          <li key={idx} className="bg-gray-900/50 border border-gray-800 rounded p-3 text-sm">
                            {blocker}
                          </li>
                        ))}
                      </ul>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-4 mb-6">
                      <pre className="text-sm text-gray-300 whitespace-pre-wrap">
                        {currentProject.monetizationPsychology}
                      </pre>
                    </div>
                    {currentProject.darkPatterns.map((pattern, idx) => (
                      <div key={idx} className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
                ...
                {activeTab === 'psychology' && activeProject === 'harvest' && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-4 mb-6">
                      <pre className="text-sm text-gray-300 whitespace-pre-wrap">
                        {currentProject.technicalDebt}
                      </pre>
                    </div>
                    {currentProject.costReality.map((item, idx) => (
                      <div key={idx} className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
                ...
                {activeTab === 'psychology' && (activeProject === 'auth' || activeProject === 'omni') && (
                  <div className="space-y-4">
                    <h4 className="text-xl font-bold mb-4">
                      {activeProject === 'auth' ? 'What Actually Works' : 'What Actually Exists'}
                    </h4>
                    {(activeProject === 'auth' ? currentProject.actualFeatures : currentProject.whatExists).map((item, idx) => (
                      <div key={idx} className="bg-gray-900/50 border border-gray-800 rounded-lg p-4">
                        <p className="text-gray-300">{item}</p>
                      </div>
                    ))}
