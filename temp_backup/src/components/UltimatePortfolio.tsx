...
function CodeEditor({ code, language = 'javascript', title = 'example.js', showLineNumbers = true }) {
  ...
  const highlightCode = (code: string) => {
    return code
      ...
      .replace(/\b(const|let|var|function|return|if|else|for|while|import|export|default|from|async|await|class|extends|implements)\b/g, '<span class="text-purple-400">$1</span>')
      .replace(/\b(true|false|null|undefined)\b/g, '<span class="text-orange-400">$1</span>')
      .replace(/\b(\d+)\b/g, '<span class="text-cyan-400">$1</span>')
      .replace(/(\/\/.*$)/gm, '<span class="text-gray-500">$1</span>')
      .replace(/(@\w+)/g, '<span class="text-yellow-400">$1</span>')
      .replace(/\b(supabase|AuthProvider|useAuth|useState|useEffect)\b/g, '<span class="text-blue-400">$1</span>');
  };
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-3 mb-3">
        <div className={`w-10 h-10 bg-gradient-to-br ${tech.gradient} rounded-lg flex items-center justify-center`}>
...
export function UltimatePortfolio() {
  ...
  const projects = {
    auth: {
      ...
      description: 'Complete SSO authentication system with subscription management, cross-domain support, and granular access control. Built for multi-product SaaS ecosystems.',
      features: ['Single Sign-On', 'Subscription Tiers', 'Cross-Domain Auth', 'PKCE Flow', 'Usage Tracking', 'TypeScript'],
      technologies: {
...
function Dashboard() {
  ...
  return (
    ...
    harvest: {
      ...
      description: 'Ethically harvests and transforms web content into structured data using AI. Features multi-agent validation, legal compliance, and semantic understanding.',
      features: ['Legal Compliance', 'Multi-Agent AI', 'Quality Validation', 'Attribution', 'Local AI', 'Rate Limiting'],
      technologies: {
...
class LegalComplianceEngine:
    ...
    quizmentor: {
      ...
      description: 'React Native learning app with advanced gamification. Features XP system, achievements, streaks, and psychological engagement patterns for 92% completion rate.',
      features: ['Gamification Engine', 'Adaptive Learning', 'Offline Support', 'Self-hosted Analytics', '60+ Animations'],
      technologies: {
...
class GamificationService {
  async awardXP(baseXP: number, reason: string) {
    ...
    omni: {
      ...
      description: 'VS Code extension that unifies all AI providers. Smart routing automatically selects the best AI for each task. 100% open source with privacy-first design.',
      features: ['Multi-Provider', 'Smart Routing', 'Hot Swapping', 'Local Models', 'Context Aware', 'Extensible'],
      technologies: {
  ...
  const technologies = [
    {
      ...
      items: ['React', 'React Native', 'Next.js', 'TypeScript', 'Tailwind', 'Framer Motion', 'Zustand']
    },
    {
      name: 'Backend',
      icon: Server,
  ...
  const blogPosts = [
    {
      ...
      excerpt: 'Deep dive into psychological patterns, XP systems, and achieving 92% completion rates through careful gamification design...',
    ...
    },
    {
      ...
      excerpt: 'How we built a legally compliant content intelligence platform that respects robots.txt, rate limits, and always attributes sources...',
    ...
    },
    {
      ...
      excerpt: 'Why we built Omni.ai to automatically select the best AI model for each task, and how it changes the development workflow...',
                  ...
                  >
                    {activeTab === 'code' && (
                      <CodeEditor 
                        ...
                        title={currentProject.name.toLowerCase().includes('harvest') ? 'compliance.py' : `${currentProject.name.toLowerCase().replace(/\s+/g, '-')}.ts`}
                        language={currentProject.name.includes('Harvest') ? 'python' : 'typescript'}
                      />
                    )}
                    {activeTab === 'terminal' && (
                      <Terminal 
                        commands={currentProject.terminal}
                        title="Terminal"
                      />
                    )}
                    {activeTab === 'technologies' && (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {Object.entries(currentProject.technologies).map(([category, techs]) => (
                          <div key={category} className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-lg p-4">
                            <h4 className="text-sm font-bold text-gray-400 mb-2 capitalize">{category}</h4>
                            <div className="space-y-1">
                              {techs.map(tech => (
                                <div key={tech} className="text-xs text-gray-300">{tech}</div>
                              ))}
                            </div>
                          </div>
                        ))}
                ...
                >
                  ...
                  <div className="relative bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-6 hover:border-gray-700 transition-all">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        onClick={() => {
                          setActiveProject(Object.keys(projects).find(k => projects[k].name === project.name));
                ...
                >
                  <article className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-6 hover:border-gray-700 transition-all">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <a href={post.link} className="flex items-center space-x-2 text-blue-400 hover:text-blue-300 transition-colors">
