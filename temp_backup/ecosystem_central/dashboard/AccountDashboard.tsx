...
import { 
  User, Shield, CreditCard, Package, Download, Trash2, 
  Settings, Activity, ChevronRight, Lock, Unlock, 
  AlertCircle, CheckCircle, Clock, TrendingUp,
  Key, Smartphone, Mail, Globe, Database, FileText,
  LogOut, RefreshCw, Zap, Star, Users, Building
} from 'lucide-react';
...
export default function AccountDashboard() {
  ...
  const products: Product[] = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      status: subscription?.tier === 'pro' || subscription?.tier === 'enterprise' ? 'active' : 'locked',
  ...
  const generateApiKey = async () => {
    try {
      ...
      setApiKeys([...apiKeys, { key: newKey, created: new Date().toISOString(), name: 'New API Key' }]);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <Badge color={product.status === 'active' ? 'green' : product.status === 'beta' ? 'yellow' : 'gray'}>
                          {product.status}
                        </Badge>
                      </div>
                  ...
                  <div key={event.id} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-0">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        {event.location && (
                          <Text className="text-gray-500 text-sm">{event.location} • {event.device}</Text>
                        )}
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {subscriptionTiers.map((tier) => (
                  ...
                  >
                    ...
                    <button className={`w-full mt-6 px-4 py-2 rounded-lg font-medium transition-all ${
                      tier.current 
            ...
            <Card>
              ...
              <div className="mt-4 space-y-2">
                {[
                  ...
                  { date: 'Nov 1, 2023', amount: '$29.00', status: 'Paid', invoice: '#INV-2023-011' }
                ].map((invoice, idx) => (
                  <div key={idx} className="flex items-center justify-between py-3 border-b border-gray-700 last:border-0">
                    <div>
                      ...
                      <Text className="text-gray-500 text-sm">{invoice.date} • {invoice.invoice}</Text>
                    </div>
                    <div className="flex items-center space-x-3">
          ...
          <div className="space-y-6">
            ...
            <Grid numItemsSm={1} numItemsLg={3} className="gap-6">
              <Card>
                <div className="flex items-center justify-between">
                  ...
                  <Shield className={`w-8 h-8 ${twoFactorEnabled ? 'text-green-400' : 'text-gray-400'}`} />
                </div>
                <button 
            ...
            {apiKeys.length > 0 && (
              <Card>
                ...
                <div className="mt-4 space-y-2">
                  {apiKeys.map((key, idx) => (
                    <div key={idx} className="p-4 bg-gray-700/30 rounded-lg flex items-center justify-between">
                      <div>
                        <Text className="font-medium text-white font-mono">{key.key.substring(0, 20)}...</Text>
                ...
                {securityEvents.map((event) => (
                  <div key={event.id} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-0">
                    <div className="flex items-center space-x-3">
                      ...
                      {event.type === 'password_change' && <RefreshCw className="w-4 h-4 text-yellow-400" />}
                      {event.type === '2fa' && <Shield className="w-4 h-4 text-blue-400" />}
                      {event.type === 'api_key' && <Key className="w-4 h-4 text-purple-400" />}
                      <div>
                        <Text className="font-medium text-white">{event.description}</Text>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Text className="mt-2 text-gray-400">
                    ...
                    This includes your profile, activity history, and all associated data.
                  </Text>
                </div>
